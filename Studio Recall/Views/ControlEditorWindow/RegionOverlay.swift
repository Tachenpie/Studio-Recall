...
//	var body: some View {
//		// Canvas-pixel geometry (no rectToPixels, do it explicitly)
//		let z = max(zoom, 0.0001)
//		let x = rect.origin.x * canvasSize.width
//		let y = rect.origin.y * canvasSize.height
//		let w = max(minRegion, rect.size.width)  * canvasSize.width
//		let h = max(minRegion, rect.size.height) * canvasSize.height
//		
//		// Screen-constant stroke + handles
//		let lw = max(0.8, min(1.8, min(w, h) / 120)) / z
//		let hs = 8.0 / z     // ~10pt on screen
//		let handleStroke = StrokeStyle(lineWidth: max(1, lw))
//		let handleFill   = Color.white
//		
//		ZStack(alignment: .topLeading) {
//			// 1) Region local container positioned at (x,y), sized (w,h)
//			ZStack(alignment: .topLeading) {
//				// 1a) Dashed outline + soft fill (drawn in LOCAL space at (0,0)-(w,h))
//				Path { p in p.addPath(pathFor(shape: shape, in: CGRect(x: 0, y: 0, width: w, height: h))) }
//					.stroke(Color.blue, style: StrokeStyle(lineWidth: lw, dash: [4,3]))
//					.background(
//						Path { p in p.addPath(pathFor(shape: shape, in: CGRect(x: 0, y: 0, width: w, height: h))) }
//							.fill(Color.blue.opacity(0.10))
//					)
//					.allowsHitTesting(false)				
//				// 1b) SINGLE hit surface for move/resize (full rect in LOCAL space)
//				Rectangle()
//					.fill(Color.clear)
//					.frame(width: w, height: h, alignment: .topLeading)
//					.contentShape(Rectangle())
//					.zIndex(1)
//				
//				// 1c) Visible handles (centers pinned explicitly so they don't drift)
//				// corners
//				Group {
//					Rectangle().fill(handleFill) // TL
//						.frame(width: hs, height: hs)
//						.overlay(Rectangle().stroke(Color.blue, style: handleStroke))
//						.position(x: hs/2, y: hs/2)
//						.allowsHitTesting(false)
//					
//					Rectangle().fill(handleFill) // TR
//						.frame(width: hs, height: hs)
//						.overlay(Rectangle().stroke(Color.blue, style: handleStroke))
//						.position(x: w - hs/2, y: hs/2)
//						.allowsHitTesting(false)
//					
//					Rectangle().fill(handleFill) // BL
//						.frame(width: hs, height: hs)
//						.overlay(Rectangle().stroke(Color.blue, style: handleStroke))
//						.position(x: hs/2, y: h - hs/2)
//						.allowsHitTesting(false)
//					
//					Rectangle().fill(handleFill) // BR
//						.frame(width: hs, height: hs)
//						.overlay(Rectangle().stroke(Color.blue, style: handleStroke))
//						.position(x: w - hs/2, y: h - hs/2)
//						.allowsHitTesting(false)
//				}
//				
//				// edge midpoints
//				Group {
//					Rectangle().fill(handleFill) // top
//						.frame(width: hs, height: hs)
//						.overlay(Rectangle().stroke(Color.blue, style: handleStroke))
//						.position(x: w/2, y: hs/2)
//						.allowsHitTesting(false)
//					
//					Rectangle().fill(handleFill) // bottom
//						.frame(width: hs, height: hs)
//						.overlay(Rectangle().stroke(Color.blue, style: handleStroke))
//						.position(x: w/2, y: h - hs/2)
//						.allowsHitTesting(false)
//					
//					Rectangle().fill(handleFill) // left
//						.frame(width: hs, height: hs)
//						.overlay(Rectangle().stroke(Color.blue, style: handleStroke))
//						.position(x: hs/2, y: h/2)
//						.allowsHitTesting(false)
//					
//					Rectangle().fill(handleFill) // right
//						.frame(width: hs, height: hs)
//						.overlay(Rectangle().stroke(Color.blue, style: handleStroke))
//						.position(x: w - hs/2, y: h/2)
//						.allowsHitTesting(false)
//				}
//			}
//			.frame(width: w, height: h, alignment: .topLeading)
